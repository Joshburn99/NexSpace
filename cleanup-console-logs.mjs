...
function removeConsoleLogs(content) {
  ...
  // Remove console.warn, console.error, console.info statements
  content = content.replace(/^\s*console\.(warn|error|info|debug)\(.*?\);?\s*$/gm, '');
  content = content.replace(/^\s*console\.(warn|error|info|debug)\([^)]*\n[^)]*\);?\s*$/gm, '');
  content = content.replace(/^\s*console\.(warn|error|info|debug)\(\s*[\s\S]*?\n\s*\);?\s*$/gm, '');
...
console.log(`\nTotal console statements removed: ~${totalRemoved} lines`);