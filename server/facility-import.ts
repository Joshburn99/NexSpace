...
export class FacilityImportService {
  ...
  private mapCMSDataToFacility(cmsData: CMSProviderData): any {
    return {
      ...
      certificationDate: cmsData.certification_date
        ? new Date(cmsData.certification_date)
        : null,
   ...
   */
  async searchAndImportFacilities(
    name: string,
    state?: string,
    city?: string,
  ): Promise<any[]> {
    ...
    for (const cmsData of cmsResults.slice(0, 10)) {
      ...
      try {
        ...
        // Check if facility already exists
        const existingFacility = await this.findFacilityByCMSId(
          cmsData.federal_provider_number,
        );
        ...
        console.error(
          `Failed to import facility ${cmsData.provider_name}:`,
          error,
        );
