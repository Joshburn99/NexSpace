...
export interface IStorage {
  ...
  createPayrollProvider(
    provider: InsertPayrollProvider,
  ): Promise<PayrollProvider>;
  ...
  createPayrollConfiguration(
    config: InsertPayrollConfiguration,
  ...
  createPayrollEmployee(
    employee: InsertPayrollEmployee,
...
export class DatabaseStorage implements IStorage {
  ...
  async getFacilityByCMSId(cmsId: string): Promise<Facility | undefined> {
    const [facility] = await db
      .select()
      .from(facilities)
      .where(eq(facilities.cmsId, cmsId));
    return facility || undefined;
  }
  ...
  async createJobApplication(
    insertApplication: InsertJobApplication,
  ): Promise<JobApplication> {
  ...
  async getFacilityStats(facilityId: number): Promise<{
    ...
    const [activeCredentials] = await db
      ...
      .innerJoin(users, eq(credentials.userId, users.id))
      .where(
        and(eq(users.facilityId, facilityId), eq(credentials.status, 'active')),
      );

    const complianceRate =
      totalCredentials.count > 0
  ...
  async createPayrollProvider(
    insertProvider: InsertPayrollProvider,
  ): Promise<PayrollProvider> {
  ...
  async getPayrollProviders(): Promise<PayrollProvider[]> {
    return await db
      ...
      .where(eq(payrollProviders.isActive, true));
  }
  ...
  async createPayrollEmployee(
    insertEmployee: InsertPayrollEmployee,
  ): Promise<PayrollEmployee> {
  ...
  async getPayrollEmployeesByFacility(
    facilityId: number,
  ): Promise<PayrollEmployee[]> {
    return await db
      .select()
  ...
  async createTimesheetEntry(
    insertEntry: InsertTimesheetEntry,
  ...
  async createPayrollSyncLog(
    insertLog: InsertPayrollSyncLog,
  ): Promise<PayrollSyncLog> {
    const [log] = await db
      .insert(payrollSyncLogs)
      .values(insertLog)
      .returning();
    return log;
  }

  async getPayrollSyncLogs(
    facilityId: number,
    providerId?: number,
  ): Promise<PayrollSyncLog[]> {
    ...
    try {
      ...
      for (const timesheet of approvedTimesheets) {
        try {
          ...
          const payrollEmployee = await this.getPayrollEmployee(
            timesheet.userId,
            facilityId,
          );
          if (!payrollEmployee) {
            errors.push(
              `No payroll configuration found for user ${timesheet.userId}`,
            );
            continue;
          }
