...
const FACILITY_USER_ROLE_TEMPLATES = {
  facility_admin: {
    ...
    permissions: [
      ...
      'view_facility_profile', 'edit_facility_profile', 'manage_facility_settings', 'manage_facility_users',
      'view_reports', 'export_data', 'view_analytics', 'view_audit_logs',
...
export async function setupFacilityUserRoleTemplates() {
  console.log('Setting up facility user role templates...');
  ...
  try {
    ...
    for (const [role, template] of Object.entries(FACILITY_USER_ROLE_TEMPLATES)) {
      ...
      if (existing.length === 0) {
        ...
        console.log(`Created role template for ${role}`);
      } else {
        ...
        console.log(`Updated role template for ${role}`);
      }
    }
    
    console.log('Facility user role templates setup complete!');
  } catch (error) {
