...
export function registerRoutes(app: Express): Server {
  ...
  app.get('/api/dashboard/stats', requireAuth, async (req: any, res) => {
    try {
      ...
      if (!facilityId) {
        return res
          .status(400)
          .json({ message: 'User not assigned to a facility' });
      }
    ...
    async (req: any, res) => {
      try {
        ...
        if (error instanceof z.ZodError) {
          res
            .status(400)
            .json({ message: 'Invalid job data', errors: error.errors });
        } else {
    ...
    try {
      const exampleShifts = [
        ...
        },
        {
          ...
          urgency: 'medium',
          description:
            'Pediatric nursing position, PALS certification preferred',
        },
        {
          id: 10,
    ...
    async (req: any, res) => {
      try {
        ...
        if (error instanceof z.ZodError) {
          res
            .status(400)
            .json({ message: 'Invalid shift data', errors: error.errors });
        } else {
    ...
    async (req: any, res) => {
      try {
        if (req.user.role !== UserRole.CONTRACTOR_1099) {
          return res
            .status(403)
            .json({ message: 'Only contractors can submit invoices' });
        }
  ...
  app.get('/api/work-logs', requireAuth, async (req: any, res) => {
    try {
      ...
      if (shiftId) {
        workLogs = await storage.getWorkLogsByShift(
    ...
    async (req: any, res) => {
      try {
        ...
        if (error instanceof z.ZodError) {
          res
            .status(400)
            .json({ message: 'Invalid work log data', errors: error.errors });
        } else {
    ...
    async (req: any, res) => {
      try {
        ...
        if (error instanceof z.ZodError) {
          res
            .status(400)
            .json({ message: 'Invalid credential data', errors: error.errors });
        } else {
  ...
  app.get('/api/messages', requireAuth, async (req: any, res) => {
    try {
      ...
      if (conversationId) {
        messages = await storage.getConversationMessages(
        ...
        switch (req.user.role) {
          case UserRole.SUPER_ADMIN:
            ...
            if (role) {
              ...
              users = await storage.getUsersByFacility(
          ...
          case UserRole.FACILITY_MANAGER:
            ...
            if (req.user.facilityId) {
              ...
              return res
                .status(403)
                .json({ message: 'Access denied: No facility assigned' });
            }
            break;
  ...
  app.patch('/api/user/:id', requireAuth, async (req: any, res) => {
    try {
      ...
      if (
        req.user.role !== UserRole.CLIENT_ADMINISTRATOR &&
        req.user.id !== userId
      ) {
        return res.status(403).json({ message: 'Insufficient permissions' });
      }
      ...
      try {
        ...
        switch (req.user.role) {
            ...
            if (userId) {
              ...
              if (
                ...
                return res
                  .status(403)
                  .json({
                    message: 'Access denied: User not in your facility',
                  });
              }
              const workLogs = await storage.getWorkLogsByUser(targetUserId);
              return res.json(workLogs);
          ...
          case UserRole.CONTRACTOR_1099:
            ...
            if (userId && parseInt(userId as string) !== req.user.id) {
              return res
                .status(403)
                .json({
                  message: 'Access denied: Can only view your own work history',
                });
            }
            break;
          ...
          default:
            ...
            console.warn(
              'Unknown user role accessing work history:',
              req.user.role,
            );
            break;
        }
  ...
  app.post('/api/create-example-facilities', async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          message: 'Failed to create example facilities',
      ...
      try {
        ...
        req.login(targetUser, (err: any) => {
          if (err) {
            return res
              .status(500)
              .json({ message: 'Failed to impersonate user' });
          }
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: 'Failed to fetch payroll configuration' });
      }
    },
  );
  ...
  app.put(
    ...
    async (req, res) => {
      try {
        ...
        const validated = insertPayrollConfigurationSchema
          .partial()
          .parse(req.body);
        ...
        res
          .status(400)
          .json({ message: 'Failed to update configuration', error });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        const facilityId = parseInt(req.params.facilityId);
        const employees =
          await storage.getPayrollEmployeesByFacility(facilityId);
        res.json(employees);
      } catch (error) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        let timesheets;
        if (userId) {
          timesheets = await storage.getTimesheetsByUser(
  ...
  app.put(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(400)
          .json({ message: 'Failed to update timesheet status', error });
      }
    },
  );
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        let payments;
        if (userId) {
          payments = await storage.getPaymentsByUser(
            parseInt(userId as string),
          );
        } else if (timesheetId) {
          payments = await storage.getPaymentsByTimesheet(
            parseInt(timesheetId as string),
          );
        } else {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const syncLog = await storage.syncWithPayrollProvider(
          facilityId,
          syncType,
        );
        res.json(syncLog);
        ...
        res
          .status(500)
          .json({ message: 'Failed to sync with payroll provider', error });
      }
    },
  );
  ...
  app.delete(
    '/api/scheduling/requirements/:id',
    requireAuth,
    async (req, res) => {
      try {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const facility =
          await facilityImportService.importFacilityByCMSId(cmsId);
        res.status(201).json(facility);
      } catch (error) {
        console.error('Error importing facility:', error);
        res
  ...
  app.post('/api/facilities/search/external', requireAuth, async (req, res) => {
    try {
      ...
      const { facilityImportService } = await import('./facility-import');
      const results = await facilityImportService.searchByNameAndLocation(
        name,
        state,
        city,
      );
        ...
        res
          ...
          .json({
            message:
              error instanceof Error
                ? error.message
                : 'Failed to refresh facility data',
          });
      }
    },
  );
  ...
  app.post('/api/facilities/recommendations', requireAuth, async (req, res) => {
    try {
      ...
      const recommendations =
        await recommendationEngine.getRecommendations(criteria);
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const recommendations =
          await recommendationEngine.getEmergencyRecommendations(
            location,
            facilityType,
          );
        res.json(recommendations);
        ...
        res
          .status(500)
          .json({ message: 'Failed to generate emergency recommendations' });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const recommendations =
          await recommendationEngine.getSpecializedCareRecommendations(
            location,
            specialty,
            maxDistance,
          );
        ...
        console.error(
          'Error generating specialized care recommendations:',
          error,
        );
        res
          .status(500)
          .json({
            message: 'Failed to generate specialized care recommendations',
          });
      }
    },
  );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (
          !insuranceType ||
          !['medicare', 'medicaid', 'both'].includes(insuranceType)
        ) {
          return res
            .status(400)
            .json({
              message:
                'Valid insurance type is required (medicare, medicaid, or both)',
            });
        }

        const recommendations =
          await recommendationEngine.getInsuranceBasedRecommendations(
            location,
            insuranceType,
            facilityType,
          );
        ...
        console.error(
          'Error generating insurance-based recommendations:',
          error,
        );
        res
          .status(500)
          .json({
            message: 'Failed to generate insurance-based recommendations',
          });
      }
    },
  );
    ...
    try {
      ...
      if (isDangerous && req.user?.role !== 'super_admin') {
        return res
          .status(403)
          .json({
            message: 'Dangerous queries require super admin privileges',
          });
      }
      ...
      const blockShifts = [
        {
          ...
          rate: 45,
          description:
            'Week-long ICU coverage needed, multiple positions available',
        },
        {
          id: 2,
  ...
  app.get(
    '/api/facility-settings/:facilityId',
    requireAuth,
    async (req, res) => {
      try {
        const settings = {
        ...
        const cleanedData = {
          ...
          dueDate:
            ...
            new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
              ...
              .split('T')[0],
          serviceDate:
            extractedData.serviceDate || new Date().toISOString().split('T')[0],
    ...
    try {
      ...
      if (specialty && specialty !== 'all') {
        filteredData = filteredData.filter(
          shift => shift.specialty === specialty,
        );
      }

      if (workerType && workerType !== 'all') {
        filteredData = filteredData.filter(
          shift => shift.workerType === workerType,
        );
      }
