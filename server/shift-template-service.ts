...
export class ShiftTemplateService {
  ...
  async generateShiftsFromTemplate(options: GenerateShiftsOptions): Promise<GeneratedShift[]> {
    const { templateId, startDate, endDate, skipExisting = true, preserveAssigned = true } = options;
  ...
  async removeDuplicateShifts(templateId?: number): Promise<number> {
    ...
    const duplicatesQuery = db
      ...
      .from(generatedShifts)
      .groupBy(
        generatedShifts.templateId,
        generatedShifts.date,
        generatedShifts.shiftPosition
      )
      .having(sql`count(*) > 1`);
   ...
   */
  async getShiftsToGenerate(daysAhead: number = 30): Promise<{ template: ShiftTemplate; dates: string[] }[]> {
