...
async function migrateFacilityUsers() {
  console.log("Starting facility users migration...");
  ...
  try {
    ...
    const facilityUserRoles = [
      ...
      'supervisor', 'director_of_nursing', 'viewer', 'corporate', 'regional_director', 
      'facility_admin'
    ];
    ...
    console.log(`Found ${facilityUsersToMigrate.length} facility users to migrate`);
    ...
    for (const user of facilityUsersToMigrate) {
      if (existingEmails.has(user.email)) {
        console.log(`Skipping ${user.email} - already exists in facility_users table`);
        continue;
      }
    ...
    if (migratedEmails.length > 0) {
      await db
        .delete(users)
        .where(inArray(users.email, migratedEmails));
      
      console.log(`Deleted ${migratedEmails.length} facility users from users table`);
    }
...
function getDefaultPermissionsForRole(role: string): string[] {
  const permissionMap: Record<string, string[]> = {
    'facility_administrator': [
      'view_facility_profile', 'edit_facility_profile', 'view_schedule', 'manage_schedule',
      'view_staff', 'manage_staff', 'view_billing', 'manage_billing', 'view_compliance',
      'manage_compliance', 'view_analytics', 'manage_teams', 'approve_timesheets',
      'manage_shift_templates', 'manage_jobs', 'view_staff_credentials', 'edit_staff_credentials',
      'manage_credentials', 'manage_users_and_teams', 'set_shift_multipliers'
    ],
    'scheduling_coordinator': [
      'view_schedule', 'manage_schedule', 'view_staff', 'manage_shift_templates',
      'view_analytics'
    ],
    'hr_manager': [
      'view_staff', 'manage_staff', 'view_compliance', 'manage_compliance',
      'view_staff_credentials', 'edit_staff_credentials', 'manage_credentials',
      'manage_jobs'
    ],
    'billing': [
      'view_billing', 'manage_billing', 'view_analytics', 'approve_timesheets'
    ],
    'supervisor': [
      'view_schedule', 'view_staff', 'approve_timesheets'
    ],
    'director_of_nursing': [
      'view_schedule', 'manage_schedule', 'view_staff', 'manage_staff',
      'view_compliance', 'manage_compliance', 'view_analytics'
    ],
    'viewer': [
      'view_facility_profile', 'view_schedule', 'view_staff'
    ],
    'corporate': [
      'view_facility_profile', 'view_schedule', 'view_staff', 'view_billing',
      'view_compliance', 'view_analytics'
    ],
    'regional_director': [
      'view_facility_profile', 'edit_facility_profile', 'view_schedule', 'view_staff',
      'view_billing', 'view_compliance', 'view_analytics', 'manage_teams'
    ]
  };
