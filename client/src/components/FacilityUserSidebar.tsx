  ...
  return (
    ...
    >
      <div className={cn("p-6", !expanded && "p-3")}>
        ...
        <nav className="space-y-1">{accessibleItems.map((item) => renderSidebarItem(item))}</nav>

        {/* Role-specific help text - only show when expanded */}
        {expanded && (
          <div className="mt-8 p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <p className="text-xs text-gray-600 dark:text-gray-400">
              Your navigation is customized based on your role permissions. Contact your
              administrator if you need access to additional features.
            </p>
          </div>
        )}
      </div>
