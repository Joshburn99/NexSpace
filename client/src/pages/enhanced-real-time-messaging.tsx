...
export default function EnhancedRealTimeMessagingPage() {
  ...
  const getUserDisplayName = (userObj: any) => {
    ...
    if (userObj.role === 'super_admin' || userObj.role === 'facility_manager') return 'NexSpace Team';
    return `${userObj.firstName || ''} ${userObj.lastName || ''}`.trim() || userObj.email || 'Unknown';
  };
  ...
  const getChatParticipants = (): ChatParticipant[] => {
    ...
    userMessages.forEach((msg: any) => {
      ...
      if (!participantMap.has(otherId)) {
        ...
        const unreadFromParticipant = userMessages.filter((m: any) => 
          m.senderId === otherId && m.recipientId === user?.id && !m.isRead
        ).length;
