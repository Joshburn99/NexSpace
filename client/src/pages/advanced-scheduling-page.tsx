...
import { Calendar, Clock, Users, MapPin, Filter, Plus, ChevronLeft, ChevronRight } from "lucide-react";
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-gray-600 dark:text-gray-300">{staff.position}</p>
                          </div>
                          <Badge 
                            className={
                              staff.availability === 'available' ? 'bg-green-100 text-green-800' :
                              staff.availability === 'limited' ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <Card className="w-96 max-h-[80vh] overflow-y-auto">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    <p className="font-medium mb-2">Assigned Staff ({selectedShift.assignedStaff.length}/{selectedShift.requiredStaff})</p>
                    <div className="space-y-2">
                      {selectedShift.assignedStaff.map(staffId => {
                        const staff = mockStaff.find(s => s.id === staffId);
                        return staff ? (
                          <div key={staffId} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-800 rounded">
                            <span className="text-sm">{staff.name} - {staff.position}</span>
                            {canAssignStaff && (
                              <Button variant="ghost" size="sm" onClick={() => {
                                ...
                                setSelectedShift({
                                  ...selectedShift,
                                  assignedStaff: selectedShift.assignedStaff.filter(id => id !== staffId)
                                });
                              }}>
                                Remove
                              </Button>
