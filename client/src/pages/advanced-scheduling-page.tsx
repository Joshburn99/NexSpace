...
export default function AdvancedSchedulingPage() {
  ...
  // Permission checks
  const canCreateShifts = hasPermission(
    user?.role || UserRole.CONTRACTOR_1099,
    'create_shifts',
  );
  const canEditShifts = hasPermission(
    user?.role || UserRole.CONTRACTOR_1099,
    'edit_shifts',
  );
  const canAssignStaff = hasPermission(
    user?.role || UserRole.CONTRACTOR_1099,
    'assign_staff',
  );
  const canViewAllShifts = hasPermission(
    user?.role || UserRole.CONTRACTOR_1099,
    'view_all_shifts',
  );
  ...
  const handleDrop = (
          ...
          setShifts(prev =>
            prev.map(shift =>
              shift.id === targetShift.id
                ? {
                    ...shift,
                    assignedStaff: [...shift.assignedStaff, staffMember.id],
                  }
                : shift,
            ),
          );
    ...
    // Handle shift creation/modification
      ...
      newEnd.setHours(
        targetHour + (shift.end.getHours() - shift.start.getHours()),
      );

      setShifts(prev =>
        prev.map(s =>
          s.id === shift.id ? { ...s, start: newStart, end: newEnd } : s,
        ),
      );
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            ...
            <Select
              value={viewMode}
              onValueChange={(value: 'week' | 'day' | 'month') =>
                setViewMode(value)
              }
            >
          ...
          <div className="lg:col-span-1">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Drag staff to assign to shifts
                </CardDescription>
              </CardHeader>
              <CardContent>
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                setSelectedShift({
                                  ...selectedShift,
                                  assignedStaff:
                                    selectedShift.assignedStaff.filter(
                                      id => id !== staffId,
                                    ),
                                });
                              }}
