...
export default function AttendancePage() {
  ...
  const filteredData = mockAttendanceData.filter(record => {
    ...
    const matchesSpecialty =
      specialty === 'all' || record.specialty === specialty;
    const matchesType =
  ...
  const specialties = Array.from(
    new Set(mockAttendanceData.map(record => record.specialty)),
  );

  const cancelRate = (
    ((attendanceStats.cancelledShifts + attendanceStats.ncnsShifts) /
      attendanceStats.totalShifts) *
    100
  ).toFixed(1);
  const lateRate = (
    (attendanceStats.lateArrivals / attendanceStats.totalShifts) *
    100
  ).toFixed(1);
    ...
    <div className="p-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-600">
                  {attendanceStats.cancelledShifts}
                </p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">
                  {attendanceStats.ncnsShifts}
                </p>
                <p className="text-xs text-gray-500">
                  {(
                    (attendanceStats.ncnsShifts / attendanceStats.totalShifts) *
                    100
                  ).toFixed(1)}
                  %
                </p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Detailed attendance data for the selected filters (
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {record.specialty} •{' '}
                      {record.employeeType === 'internal' ? 'Internal' : 'PRN'}
                    </p>
                  </div>
                </div>
                  ...
                  {record.actualClockIn ? (
                    <>
                      ...
                      <p className="text-sm">
                        ...
                        {record.actualClockOut || 'Active'}
                      </p>
                      {record.minutesLate !== 0 && (
                        <p
                          className={`text-xs ${record.minutesLate > 0 ? 'text-red-500' : 'text-blue-500'}`}
                        >
                          {record.minutesLate > 0 ? '+' : ''}
