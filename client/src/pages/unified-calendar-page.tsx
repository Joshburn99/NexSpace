...
export default function UnifiedCalendarPage() {
  ...
  const postShiftMutation = useMutation({
    mutationFn: async (shiftData: any) => {
      const endpoint = shiftData.isBlockShift
        ? '/api/block-shifts'
        : '/api/shifts';
  ...
  const handlePremiumMultiplierChange = (multiplier: number) => {
    const baseRate =
      BASE_RATES[shiftForm.specialty as keyof typeof BASE_RATES] || 35;
    setShiftForm(prev => ({
      ...prev,
  ...
  const handleSubmitShift = () => {
    ...
    const shiftData = {
      ...shiftForm,
      facilityName:
        facilities.find((f: any) => f.id === shiftForm.facilityId)?.name ||
        'Unknown Facility',
    };
  ...
  const getSpecialtyBadgeColor = (specialty: string) => {
    const colors: { [key: string]: string } = {
      ...
      'Nurse Practitioner':
        'bg-gradient-to-r from-pink-500 to-pink-600 text-white shadow-pink-300',
      'Physician Assistant':
        'bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-orange-300',
    };
    return (
      colors[specialty] ||
      'bg-gradient-to-r from-gray-500 to-gray-600 text-white shadow-gray-300'
    );
  };
          ...
          {viewMode === 'week' ? (
                      ...
                      {dayShifts.slice(0, 3).map((shift: Shift) => (
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between mt-1">
                            ...
                            <Badge
                              className={`text-xs px-1 py-0 ${getStatusColor(shift.status)}`}
                            >
                              {shift.status === 'open'
                                ? 'OPEN'
            ...
            <div className="grid grid-cols-7 gap-1">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                <div
                  key={day}
                  className="p-2 text-center font-semibold text-sm border-b"
                >
                  {day}
                </div>
              ))}
            ...
            <div className="space-y-4">
              ...
              {selectedShift.urgency && (
                <div>
                  ...
                  >
                    {selectedShift.urgency.charAt(0).toUpperCase() +
                      selectedShift.urgency.slice(1)}
                  </Badge>
                </div>
              )}
            </div>
      ...
      <Dialog open={showPostingDialog} onOpenChange={setShowPostingDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh]">
          ...
          <ScrollArea className="max-h-[70vh] pr-4">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={e =>
                      setShiftForm(prev => ({ ...prev, title: e.target.value }))
                    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <Label>
                    {shiftForm.isBlockShift ? 'Start Date *' : 'Date *'}
                  </Label>
                  <Input
                    type="date"
                    value={shiftForm.date}
                    onChange={e =>
                      setShiftForm(prev => ({ ...prev, date: e.target.value }))
                    }
                  />
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <Label>
                    Premium Multiplier: {shiftForm.premiumMultiplier.toFixed(1)}
                    x
                  </Label>
                  <Slider
                    value={[shiftForm.premiumMultiplier]}
                    onValueChange={value =>
                      handlePremiumMultiplierChange(value[0])
                    }
                    max={2.0}
                    min={1.0}
                    step={0.1}
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            <Button
              variant="outline"
              onClick={() => setShowPostingDialog(false)}
            >
              Cancel
            </Button>
            <Button
