...
export default function AgencyUsagePage() {
  ...
  const totalAgencySpend = mockAgencyData.reduce(
    (sum, agency) => sum + agency.totalSpent,
    0,
  );
  const totalFloatPoolCost = floatPoolData.reduce(
    (sum, pool) => sum + pool.totalCost,
    0,
  );
  const totalFloatPoolSavings = floatPoolData.reduce(
    (sum, pool) => sum + pool.savingsVsAgency,
    0,
  );

  const averageAgencyRate =
    mockAgencyData.reduce((sum, agency) => sum + agency.averageRate, 0) /
    mockAgencyData.length;
  const averageMarketRate =
    mockAgencyData.reduce((sum, agency) => sum + agency.marketRate, 0) /
    mockAgencyData.length;
  ...
  const specialties = Array.from(
    new Set(mockAgencyData.map(agency => agency.specialty)),
  );
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  ${totalAgencySpend.toLocaleString()}
                </p>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-bold">
                        {pool.hoursWorked.toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-600 dark:text-gray-300">
        ...
        <CardContent>
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-bold">
                        ${agency.totalSpent.toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-600 dark:text-gray-300">Hours</p>
                      <p className="font-bold">
                        {agency.hoursWorked.toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-600 dark:text-gray-300">
          ...
          <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            ...
            <ul className="text-sm space-y-1">
              <li>
                • Increase float pool utilization by 15% to save an estimated
                $8,500/month
              </li>
              <li>
                • Negotiate Premium Healthcare Staffing rates - currently 15.6%
                above market
              </li>
              <li>
                • Consider expanding cross-training program for internal staff
              </li>
              <li>
                • Evaluate Elite Therapy Staff performance vs cost premium
              </li>
            </ul>
          </div>
        </CardContent>
