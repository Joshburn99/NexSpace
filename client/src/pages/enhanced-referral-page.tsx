...
export default function EnhancedReferralPage() {
  ...
  const [showFacilityReferralDialog, setShowFacilityReferralDialog] =
    useState(false);
  const [showQRDialog, setShowQRDialog] = useState(false);
  const [selectedReferralCode, setSelectedReferralCode] =
    useState<ReferralCode | null>(null);

  const { data: referralSettings, isLoading: settingsLoading } =
    useQuery<ReferralSettings>({
      queryKey: ['/api/referral-settings'],
    });

  const { data: staffReferrals = [], isLoading: staffReferralsLoading } =
    useQuery<StaffReferral[]>({
      queryKey: ['/api/referrals/staff'],
    });

  const { data: facilityReferrals = [], isLoading: facilityReferralsLoading } =
    useQuery<FacilityReferral[]>({
  ...
  const handleUpdateSettings = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const settings = {
      staffReferralBonus: parseFloat(
        formData.get('staffReferralBonus') as string,
      ),
      facilityReferralBonus: {
        ...
        enterprise: parseFloat(
          formData.get('enterpriseFacilityBonus') as string,
        ),
      },
      qualificationPeriod: parseInt(
        formData.get('qualificationPeriod') as string,
      ),
  ...
  const handleCreateFacilityReferral = (
    ...
    e.preventDefault();
    const formData = new FormData(e.currentTarget);
    const facilitySize = formData.get(
      'facilitySize',
    ) as keyof typeof facilitySizeLabels;
    const referralData = {
      referrerId: parseInt(formData.get('referrerId') as string),
      ...
      notes: formData.get('notes') as string,
      bonusAmount:
        referralSettings?.facilityReferralBonus[facilitySize] || 1000,
    };
  ...
  if (
    settingsLoading ||
    staffReferralsLoading ||
    facilityReferralsLoading ||
    codesLoading
  ) {
    return (
              ...
              <DialogContent className="max-w-2xl">
                ...
                <form onSubmit={handleUpdateSettings} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="staffReferralBonus">
                    ...
                    <div>
                      <Label htmlFor="qualificationPeriod">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-2">
                      <div>
                        <Label htmlFor="smallFacilityBonus">
                          Small Facility ($)
                        </Label>
                        <Input
                          ...
                          step="0.01"
                          defaultValue={
                            referralSettings?.facilityReferralBonus.small
                          }
                          required
                        />
                      </div>
                      <div>
                        <Label htmlFor="mediumFacilityBonus">
                          Medium Facility ($)
                        </Label>
                        <Input
                          ...
                          step="0.01"
                          defaultValue={
                            referralSettings?.facilityReferralBonus.medium
                          }
                          required
                        />
                      </div>
                      <div>
                        <Label htmlFor="largeFacilityBonus">
                          Large Facility ($)
                        </Label>
                        <Input
                          ...
                          step="0.01"
                          defaultValue={
                            referralSettings?.facilityReferralBonus.large
                          }
                          required
                        />
                      </div>
                      <div>
                        <Label htmlFor="enterpriseFacilityBonus">
                          Enterprise Facility ($)
                        </Label>
                        <Input
                          ...
                          step="0.01"
                          defaultValue={
                            referralSettings?.facilityReferralBonus.enterprise
                          }
                          required
                        />
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        name="payoutSchedule"
                        defaultValue={referralSettings?.payoutSchedule}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <div>
                      <Label htmlFor="minimumShifts">
                        Minimum Shifts for Qualification
                      </Label>
                      <Input
                        name="minimumShifts"
                        type="number"
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {facilityStats.signed} signed • ${facilityStats.totalBonus}{' '}
                  earned
                </p>
              </div>
            </div>
      ...
      <Tabs defaultValue="staff" className="space-y-6">
        ...
        <TabsContent value="staff" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Dialog
              open={showStaffReferralDialog}
              onOpenChange={setShowStaffReferralDialog}
            >
              <DialogTrigger asChild>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Refer a new staff member to earn $
                    {referralSettings?.staffReferralBonus}
                  </DialogDescription>
                </DialogHeader>
                <form
                  onSubmit={handleCreateStaffReferral}
                  className="space-y-4"
                >
                  <div>
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button
                      type="submit"
                      disabled={createStaffReferralMutation.isPending}
                    >
                      Create Referral
                    </Button>
                  </div>
                ...
                <TableBody>
                  {staffReferrals.map(referral => (
                    <TableRow key={referral.id}>
                      ...
                      <TableCell>
                        <Badge
                          className={`${statusColors[referral.status]} text-white`}
                        >
        ...
        <TabsContent value="facilities" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Dialog
              open={showFacilityReferralDialog}
              onOpenChange={setShowFacilityReferralDialog}
            >
              <DialogTrigger asChild>
                <Button className="gap-2">
                  <Plus className="h-4 w-4" />
              ...
              <DialogContent className="max-w-2xl">
                ...
                <form
                  onSubmit={handleCreateFacilityReferral}
                  className="space-y-4"
                >
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      ...
                      <Select name="facilitySize" required>
                        ...
                        <SelectContent>
                          {Object.entries(facilitySizeLabels).map(
                            ([value, label]) => (
                              <SelectItem key={value} value={value}>
                                {label}
                              </SelectItem>
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button
                      type="submit"
                      disabled={createFacilityReferralMutation.isPending}
                    >
                      Create Referral
                    </Button>
                  </div>
                    ...
                    <TableRow key={referral.id}>
                      ...
                      <TableCell>
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            {facilityTypeLabels[referral.facilityType]} •{' '}
                            {referral.estimatedBeds} beds
                          </div>
                      ...
                      <TableCell>
                        ...
                        >
                          {referral.status
                            .replace('_', ' ')
                            .charAt(0)
                            .toUpperCase() + referral.status.slice(1)}
                        </Badge>
                      </TableCell>
                      <TableCell>${referral.bonusAmount}</TableCell>
                      <TableCell>
        ...
        <TabsContent value="qr-codes" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Button
              className="gap-2"
              onClick={() =>
                generateQRCodeMutation.mutate({ userId: 1, type: 'both' })
              }
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="flex-1"
                      onClick={() =>
                        downloadQRCode(code.qrCodeUrl, `qr-${code.code}`)
                      }
                    >
