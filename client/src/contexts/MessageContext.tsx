...
const sampleMessages: Message[] = [
  ...
  },
  {
    ...
    content:
      "Congratulations to our ICU team for achieving the highest patient satisfaction scores this quarter! 🎉",
    timestamp: new Date(Date.now() - 1 * 60 * 60 * 1000).toISOString(),
    attachments: [
      {
        id: "att-1",
...
export const MessageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const sendMassMessage = (recipientIds: number[], content: string, subject?: string) => {
    ...
    recipientIds.forEach((recipientId) => {
      ...
      const threadId = createThread("direct", getDisplayName(recipientId, user, staff), [
        user.id,
        recipientId,
      ]);
      ...
      const newMessage: Message = {
        ...
        senderId: user.id,
        senderName:
          user.role === "superuser" ? "NexSpace Team" : `${user.firstName} ${user.lastName}`,
