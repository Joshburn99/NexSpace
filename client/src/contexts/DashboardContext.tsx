...
export const DashboardProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const todayRequestedShifts = requestedShifts.filter(
    (shift) => shift.date && shift.date.startsWith(today)
  );
  const todayBookedShifts = bookedShifts.filter(
    (shift) => shift.date && shift.date.startsWith(today)
  );
  const todayCompletedShifts = completedShifts.filter(
    (shift) => shift.date && shift.date.startsWith(today)
  );
  ...
  const dashboardData: DashboardData = {
    totalShiftsToday:
      todayOpenShifts.length + todayRequestedShifts.length + todayBookedShifts.length,
