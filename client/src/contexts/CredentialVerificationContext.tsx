...
const CredentialVerificationContext = createContext<CredentialVerificationContextType | undefined>(
  undefined
);
...
export const CredentialVerificationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const getVerifiedCredentials = (staffId: number): VerifiedCredential[] => {
    return verifiedCredentials.filter(
      (cred) => cred.staffId === staffId && cred.verificationStatus === "verified"
    );
  };
...
export const useCredentialVerification = (): CredentialVerificationContextType => {
  ...
  if (!context) {
    throw new Error(
      "useCredentialVerification must be used within a CredentialVerificationProvider"
    );
  }
  return context;
};
