    ...
    return (
      <div className="relative inline-block group">
        ...
        >
          ...
          {showLockIcon && (
            <svg
              className="w-3 h-3 absolute top-0 right-0 -mt-1 -mr-1"
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path
...
export function RoleBadge({ role }: { role?: SystemRole }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${metadata.color}`}
    >
      {metadata.label}
    </span>
  );
...
export function PermissionIndicator({ permissions }: { permissions: Permission[] }) {
  const rbac = useRBAC();
