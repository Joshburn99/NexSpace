...
export const useShiftsWithAssignments = () => {
  ...
  const getShiftWithStaffing = (shiftId: string): ShiftWithStaffing | null => {
    ...
    const assignedWorkers = usersData.filter((user: User) => assignedWorkerIds.includes(user.id));

    // Get requested workers (currently not implemented in this hook)
    const requestedWorkers: User[] = [];
